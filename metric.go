...
// StreamTag values represent stream tags.
...
func NewMetricName() *MetricName {
	return &MetricName{StreamTags: []StreamTag{}}
}
...
func (mp *MetricParser) Parse() (*MetricName, error) {
	...
	if tok == tokenStreamTag {
		...
		for {
			...
			if err != nil {
				return nil, fmt.Errorf("unable to parse stream tag name: %w", err)
			}
			...
			if err != nil {
				return nil, fmt.Errorf("unable to parse stream tag value: %w", err)
			}

			if tok != tokenTagVal {
				return nil, fmt.Errorf("expected stream tag value, got: %s", lit)
			}
			...
			canonical.WriteString(lit)
			metricName.StreamTags = append(metricName.StreamTags, tag)
			tok, lit = mp.s.scan()
			if tok == tokenComma {
		...
		return nil, fmt.Errorf("measurement tag not implemented")
	}
...
// ParseMetricName takes a cannonical metric name as a string and parses it
